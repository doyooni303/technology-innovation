...
try:
    ...
    from ..query_analyzer import QueryAnalysisResult, QueryComplexity, QueryType, SearchMode
...
class SubgraphExtractor:
    ...
    def load_unified_graph(self) -> Dict[str, Any]:
        ...
        logger.info(f"✅ Graph loaded: {len(self.graph_data['nodes'])} nodes, {len(self.graph_data['edges'])} edges")
        return self.graph_data
    ...
    def load_vector_store(self) -> VectorStoreManager:
        ...
        if (self.vector_store_path / "chroma.sqlite3").exists():
            store_type = "chroma"
        elif (self.vector_store_path / "faiss_index.bin").exists():
            store_type = "faiss"
        else:
            store_type = "auto"
        ...
        logger.info(f"✅ Vector store loaded: {store_type}")
        return self.vector_store
    ...
    def extract_subgraph(
        ...
        # 캐시 확인
        cache_key = f"{query}_{strategy}_{hash(str(config.__dict__))}" if self.query_cache else None
        ...
        try:
            ...
            logger.info(f"✅ Subgraph extracted: {result.total_nodes} nodes, {result.total_edges} edges")
    ...
    def _determine_strategy(self, query_analysis: Optional[QueryAnalysisResult]) -> SearchStrategy:
    ...
    def _initial_vector_search(
        ...
        logger.info(f"✅ Found {len(filtered_results)} initial matches (threshold: {config.similarity_threshold})")
    ...
    def _expand_from_initial_matches(
        ...
        if strategy == SearchStrategy.LOCAL:
            ...
            expanded_nodes = self._community_expansion(seed_nodes, config, expansion_path)
        elif strategy == SearchStrategy.TEMPORAL:
            expanded_nodes = self._temporal_expansion(seed_nodes, query_analysis, config, expansion_path)
        elif strategy == SearchStrategy.SEMANTIC:
            expanded_nodes = self._semantic_expansion(seed_nodes, config, expansion_path)
        else:  # HYBRID
            expanded_nodes = self._hybrid_expansion(seed_nodes, query_analysis, config, expansion_path)
    ...
    def _global_expansion(
        ...
        if not hasattr(self, '_centrality_scores'):
            ...
            self._centrality_scores = nx.degree_centrality(self.networkx_graph.to_undirected())
        ...
        sorted_nodes = sorted(
            all_nodes,
            key=lambda x: self._centrality_scores.get(x, 0),
            reverse=True
        )
        ...
        for node_id in sorted_nodes:
            ...
            for existing_node in expanded:
                if (self.networkx_graph.has_edge(node_id, existing_node) or
                    self.networkx_graph.has_edge(existing_node, node_id)):
                    has_connection = True
                    break
    ...
    def _semantic_expansion(
        ...
        for result in similar_results:
            ...
            if (result.similarity_score >= config.similarity_threshold and
                result.node_id not in expanded):
    ...
    def _hybrid_expansion(
        ...
        for strategy, weight in strategies:
            ...
            if strategy == SearchStrategy.LOCAL:
                strategy_result = self._local_expansion(current_expanded, temp_config, temp_expansion_path)
            elif strategy == SearchStrategy.SEMANTIC:
                strategy_result = self._semantic_expansion(current_expanded, temp_config, temp_expansion_path)
            elif strategy == SearchStrategy.GLOBAL:
                strategy_result = self._global_expansion(current_expanded, temp_config, temp_expansion_path)
            else:
    ...
    def _build_subgraph(
        ...
        logger.info(f"✅ Subgraph built: {len(subgraph_nodes)} nodes, {len(subgraph_edges)} edges")
    ...
    def _calculate_relevance_scores(
        ...
        # 초기 매치들의 점수
        initial_scores = {result.node_id: result.similarity_score for result in initial_matches}
